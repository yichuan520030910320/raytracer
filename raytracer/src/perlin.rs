...
impl Perlin {
    ...
    pub fn noise(&self, p: Vec3) -> f64 {
        ...
        let mut my_sz = [[[0.0; 2]; 2]; 2];
        for di in 0..2 {
            for dj in 0..2 {
                for dk in 0..2 {
                    my_sz[di][dj][dk] = self.ranfloat[
                       ( (self.perm_x[((i + (di as i32)) & 255)as usize] as usize)^
                           ( self.perm_y[((j + (dj as i32)) & 255)as usize]as usize )^
                            (self.perm_z[((k + (dk as i32)) & 255)as usize]as usize))as usize
                        ];
                }
            }
        }
        return Perlin::trilinear_interp(my_sz,u,v,w);
    ...
    pub fn trilinear_interp(mut my_sz: [[[f64; 2]; 2]; 2], u: f64, v: f64, w: f64) -> f64 {
        let mut accum = 0.0;
        for i in 0..2 {
            for j in 0..2 {
                for k in 0..2 {
                    accum += ((i as f64) * u + (((1 - i))as f64) * (1.0 - u)) * ((j as f64* v) + ((1 - j)as f64) * (1.0 - v) )* ((k as f64)* w + (((1 - k))as f64) * (1.0- w)) * my_sz[i][j][k];
                }
            }
        }
        return accum;
