        ...
        for i in 0..2 {
            for j in 0..2 {
                for k in 0..2 {
                    ...
                    accum += ((i as f64) * uu + (((1 - i))as f64) * (1.0 - uu)) * ((j as f64* vv) + ((1 - j)as f64) * (1.0 - vv) )* ((k as f64)* ww + (((1 - k))as f64) * (1.0- ww)) * (Vec3::dot(my_sz[i][j][k],weight_v));
                }
            }
        }
        return accum;
...
impl Texture for NoiseTexture {
    fn value(&self, u: f64, v: f64, p: &Vec3) -> Vec3 {
        return Vec3::new(1.0, 1.0, 1.0) *0.5*((self.scale*p.z+10.0*self.noise.turb(*p,0)).sin()+1.0);
